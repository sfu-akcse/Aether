...
def is_grabbing(hand_landmarks):
    ...
    curled_fingers = 0
    finger_pairs = [
        (8, 5),   # index tip vs index MCP
        (12, 9),  # middle tip vs middle MCP
        (16, 13), # ring tip vs ring MCP
        (20, 17), # pinky tip vs pinky MCP
    ]

    # Use 3D fingertip-to-wrist distance so wrist pitch does not shrink the
    # hand in screen space and accidentally trigger a grab.
    for tip_index, mcp_index in finger_pairs:
        tip_distance = distance_3d(hand_landmarks[tip_index], wrist) / hand_size
        mcp_distance = distance_3d(hand_landmarks[mcp_index], wrist) / hand_size
        if tip_distance < mcp_distance + 0.55:
            curled_fingers += 1

    thumb_to_index = distance_3d(hand_landmarks[4], hand_landmarks[8]) / hand_size

    return curled_fingers >= 3 and thumb_to_index < 1.1
    ...
    try:
        while True:
            ...
            if frame is None:
                ...
                cv2.imshow("Grab Detection 2", status_frame)
            ...
            cv2.imshow("Grab Detection 2", frame)
