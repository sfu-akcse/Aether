...
from motion_recorder import FIELDNAMES, build_row, default_output_path, snap_degrees

# Dual-hand tracking pipeline (same as main2.py) plus a recording key ('s')
# that saves each frame's data to a CSV via motion_recorder.build_row.
# main.py/main2.py are unmodified — this is a separate entry point.

logger = setup_logger("aether-system.log", "AETHER.VISION.MAIN_RECORDER")
...
def main():
    logger.info("Starting Aether dual-hand pipeline with recording.")
    ...
    try:
        ...
        print("[KEYS] b = calibrate left wrist | r = set right-hand Z=0 | s = start/stop recording | ESC = quit")
        ...
        while True:
            ...
            if image is None:
                if not headless:
                    ...
                    cv2.imshow("Aether Main Recorder", status)
            ...
            summary_lines = [
                ...
                f"Recording: {'ON (' + str(frames_written) + ')' if recording else 'OFF (press s)'}",
            ]
            ...
            if recording:
                ...
                if left_wrist is not None:
                    wrist_for_csv = {
                        "roll_deg": snap_degrees(left_wrist["roll_delta"]),
                        "pitch_deg": snap_degrees(left_wrist["pitch_delta"]),
                    }
                grab_for_csv = left_grab if left_state is not None else None
                elapsed = time.monotonic() - record_start
                csv_writer.writerow(build_row(elapsed, right_xyz, right_z, wrist_for_csv, grab_for_csv))
                frames_written += 1

            if not headless:
                cv2.imshow("Aether Main Recorder", image)
                ...
                if key == ord("s"):
                    ...
                    if recording:
                        ...
                        csv_file.close()
                        csv_file = None
                        print(f"[REC] Recording stopped. {frames_written} frames written to {output_path}")

        return 0
    except KeyboardInterrupt:
        logger.info("Keyboard interrupt received. Shutting down main_recorder.")
        return 0
    except Exception:
        logger.exception("main_recorder terminated due to an unexpected error.")
        return 1
    finally:
