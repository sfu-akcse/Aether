...
# GrabbingMotion_sideview algorithms start here
...
def sideview_is_grabbing(hand_landmarks, was_grabbing):
    ...
    hand_size = distance_3d(wrist, middle_mcp)

    if not math.isfinite(hand_size) or hand_size <= 1e-6:
        return False
    
    # Normalize fingertip to palm
    thumb_to_palm = distance_3d(hand_landmarks[4], hand_landmarks[2]) / hand_size
    index_to_palm = distance_3d(hand_landmarks[8], hand_landmarks[5]) / hand_size
    middle_to_palm = distance_3d(hand_landmarks[12], hand_landmarks[9]) / hand_size
    ring_to_palm = distance_3d(hand_landmarks[16], hand_landmarks[13]) / hand_size
    pinky_to_palm = distance_3d(hand_landmarks[20], hand_landmarks[17]) / hand_size

    if was_grabbing:
        curl_threshold = OPEN_THRESHOLD
    else:
        curl_threshold = CLOSED_THRESHOLD

    # How to know whether the finger is curled
    thumb_curled = thumb_to_palm < curl_threshold
    index_curled = index_to_palm < curl_threshold
    middle_curled = middle_to_palm < curl_threshold
    ring_curled = ring_to_palm < curl_threshold
    pinky_curled = pinky_to_palm < curl_threshold

    curled_finger_count = 0

    if thumb_curled:
        curled_finger_count += 1
...
def main():
    ...
    # --- starting the was_grabbed logic --- 
    last_status = "Open"

    # Added
    last_hand_seen_time = time.monotonic()
    ...
    try:
        while True:
            ...
            if frame is None:
                ...
                cv2.imshow("Grab Detection Side View", status_frame)
            ...
            frame_age = current_time - latest_ts
            status = last_status
            ...
            if frame_age <= 1.0:
                ...
                if result.hand_landmarks:
                    ...
                    if last_status == "Grabbing":
                        was_grabbing = True
                    else:
                        was_grabbing = False

                    grabbing_now = sideview_is_grabbing(hand_landmarks, was_grabbing)
                    ...
                    hand_missing_duration = current_time - last_hand_seen_time

                    if hand_missing_duration >= HAND_LOST_TIMEOUT:
                        status = "Open"
                    
                    cv2.putText(frame, "Hand not detected", (20, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 165, 255), 2)
            else:
                status = "Open"
            ...
            cv2.imshow("Grab Detection Side View", frame)
